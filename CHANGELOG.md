...
change — a suite that never fails is not evidence of anything. The file admits 958 mutations,
...
A full pass has been run: **958 mutations, 954 killed, none survived.** The file has been
edited since, so the count is the same and the file is not; it is re-run rather than restated.
